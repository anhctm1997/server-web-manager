...
import { IsAdmin } from "./common.permissionslevel.type";
import messageStatus from "../../configs/constant/messageStatus";
import debug from "debug";
const log: debug.IDebugger = debug("app:common-permission-middleware");
class CommonPermissionsMiddlewares {
  memberPermissionLevelRequire(requiredPermissionLevel: IsAdmin) {
    return (
      req: express.Request,
  ...
  async onlySameUserOrAdminCanDoThisAction(
    ...
    if (
      ...
      if (userPermissionLevel & IsAdmin.ADMIN_PERMISSION) {
        return next();
      } else {
